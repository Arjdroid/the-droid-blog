...
Suppose you've eaten ![equation](math/eq_1.svg) slices so far, where ![equation](math/eq_2.svg). You
then face the following choices:

1.  Eat the ![equation](math/eq_3.svg) apple slice

2.  Reject the ![equation](math/eq_4.svg) apple slice
...
![equation](math/eq_5.svg).

However, if we consider that for some finite natural number ![equation](math/eq_6.svg),
![equation](math/eq_7.svg), and that you always
prefer to have more apple slices than you already have, it seems very
compelling to say that
![equation](math/eq_8.svg).
So, for every possible ![equation](math/eq_9.svg), it is always preferrable to take ![equation](math/eq_10.svg)
apple slices over merely ![equation](math/eq_11.svg) since just one more apple slice does not
...
synchronically seem rational (such as taking ![equation](math/eq_12.svg) slices) ends up
...
actions as being either ![equation](math/eq_13.svg)
...
If we assign ![equation](math/eq_14.svg),
![equation](math/eq_15.svg), and
![equation](math/eq_16.svg), then the utility of
![equation](math/eq_17.svg) while
![equation](math/eq_18.svg).
...
decision (say if only ![equation](math/eq_19.svg) and
![equation](math/eq_20.svg) utility instead). Thus, it seems
...
![equation](math/eq_21.svg) apple slices and guaranteeing heaven is the status quo so
![equation](math/eq_22.svg), and ![equation](math/eq_23.svg).
The actual number doesn't matter as long as
![equation](math/eq_24.svg).
![equation](math/eq_25.svg) so that
![equation](math/eq_26.svg).
With these we get the utility of taking all to be
![equation](math/eq_27.svg).
...
slice. For example,
![equation](math/eq_28.svg)
whereas
![equation](math/eq_29.svg)
The reason diminishing marginal utility alone doesn't give a rationale
for stopping at any ![equation](math/eq_30.svg) is that the marginal utility never goes below
...
1.  First assign credences (degrees of belief) ![equation](math/eq_31.svg) to the occurrence
    of all possible sequences ![equation](math/eq_32.svg).\
    One possible way of assigning credences is based on cardinalities as
    follows:

    ![equation](math/eq_33.svg)

    Since ![equation](math/eq_34.svg) occurring depends on choosing to continue at each
    previous state ![equation](math/eq_35.svg)

2.  For each ![equation](math/eq_36.svg), compute expected utilities for the two choices:

    ![equation](math/eq_37.svg)
    ...
    ![equation](math/eq_38.svg), more precise
    than ![equation](math/eq_39.svg). For
    ![equation](math/eq_40.svg) the relevant probabilities are:

    ![equation](math/eq_41.svg)

    ![equation](math/eq_42.svg)

    The status quo will have the following expected utility at the
    outset:

    ![equation](math/eq_43.svg)

<!-- -->

1.  Revision of each credence ![equation](math/eq_44.svg) is done as per the following
    revision rule ![equation](math/eq_45.svg)//, applied ![equation](math/eq_46.svg) ?sequentially over and
    over? as part of the deliberation process
    ![equation](math/eq_47.svg)
...
2.  The agent stops deliberating after reaching an equilibrium (the
    ...
    ![equation](math/eq_48.svg)

## Rational credences

The way deliberative dynamics works, the only initial credences that
reach any equilibria are ![equation](math/eq_49.svg) and ![equation](math/eq_50.svg) [@bartha_satan_2014], since those
...
credence ![equation](math/eq_51.svg), could lead to an equilibrium credence of ![equation](math/eq_52.svg), and vice
...
have a set of credences (like ![equation](math/eq_53.svg)) that would
lead me to perform directly contradictory actions i.e, taking every
apple slice while believing I certainly won't.

It is also pragmatically undesirable since ![equation](math/eq_54.svg)
will guarantee the lowest utility outcome of hell. On this basis,
holding credence ![equation](math/eq_55.svg) for any ![equation](math/eq_56.svg) seems undesirable as it would
...
credences ![equation](math/eq_57.svg), are they unable to reach any
...
their own risk function ![equation](math/eq_58.svg) which applies to their probabilities and
maps them to a value still between ![equation](math/eq_59.svg) and ![equation](math/eq_60.svg). This risk function
...
that surely pays out ![equation](math/eq_61.svg) over one that has a ![equation](math/eq_62.svg) chance of
paying ![equation](math/eq_63.svg) and a ![equation](math/eq_64.svg) chance of paying ![equation](math/eq_65.svg), even though
...
that even extremely low probabilities are mapped to ![equation](math/eq_66.svg) in their
...
Suppose an agent doesn't hold ![equation](math/eq_67.svg) at the start of
...
of Satan's Apple that choosing ![equation](math/eq_68.svg) slices over ending at ![equation](math/eq_69.svg) slices
...
![equation](math/eq_70.svg) slice.

An extreme risk function that assigns every non-zero credence ![equation](math/eq_71.svg)
so only ![equation](math/eq_72.svg) would be convenient for this purpose. However,
[@buchak_risk_2009] suggests that ![equation](math/eq_73.svg) must be a continuous function. If
that is the case, it makes the task more difficult. A continuous
function that assigns every credence ![equation](math/eq_74.svg) a risk-weighted
credence of ![equation](math/eq_75.svg) seems undesirable, since a credence of ![equation](math/eq_76.svg) gives
...
continuous, and does not map every possible credence onto ![equation](math/eq_77.svg), then we
